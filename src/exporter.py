...
def generate_html_report(trips, config, output_path, period_str, map_cache_dir=None):
    ...
    """
    # Beräkningar - FIX: Ändrat från 'distance_km' till 'Km'
    total_km = sum(float(t.get("Km", 0)) for t in trips)
    ...
    for t in trips:
        # FIX: Hämta tiden med rätt nyckel
        start_t = t.get("Start", "---")
        ...
        # Hantera Mätarställning (ODO) - FIX: Konvertera säkert till float
        odo_start = float(t.get("Start_Odo", 0) or 0)
        odo_end = float(t.get("End_Odo", 0) or 0)
        ...
        # Hantera Karta - FIX: Rätt nyckel och id/temp_id
        coords = t.get("route_coords", [])
        trip_id = t.get("id") or t.get("temp_id", "unknown")
        ...
        # Körtid - FIX: Rätt nyckel
        tid = t.get("Total_Tid", "0m")

        # Avstånd (km) - FIX: Rätt nyckel
        km_val = float(t.get("Km", 0))
